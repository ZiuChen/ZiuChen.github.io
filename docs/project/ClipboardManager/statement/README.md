...
- Q: 为什么要取消收藏功能?
- A: 收藏功能没有移除，只是移除了插件内的收藏栏，届时点击收藏按钮将自动携带数据跳转到备忘快贴，以此实现收藏功能。这是出于以下原因考虑的:
  - 专门的事情还是交给专门的插件来处理，而不是所有功能都 all-in-one
  - 目前剪贴板的收藏功能有很多局限性，收藏列表的后续维护也比较困难
  - `备忘快贴`提供的收藏功能体验很优秀，远优于`超级剪贴板`内置的收藏功能
  - 两个插件之间跳转的体验是连贯的，这一点可以在`v1.3.1`及以上的“分词”功能中体现

然而临时需要高频使用某条内容的需求是存在的，所以后续可能会添加一个类似`临时置顶`的功能，置顶的历史记录仅在当前运行周期有效，下次启动将自动清空。
